...
from eval import eval_nll, eval_one_step_predict, eval_intensity
import numpy as np
...
path = './my_datasets'
with open(f'{path}/{args.dataname}/poisson.npy', 'rb') as f:
    params = np.load(f)
    mus = jnp.asarray(params['mus'])
    t_max = params['t_max']
poisson = Poisson(mus)
mape = eval_intensity(model, poisson, test_loader, intensity_key, scale, 2000, t_max)
print(f'mape={mape}')